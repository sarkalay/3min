...
class Aggressive15MinScalpingBot:
    ...
    def execute_trade(self, decision):
        ...
        try:
            ...
            try:
                ...
                self.print_color(f"🔥 15MIN LIVE TRADE ACTIVATED: {pair} {direction}", self.Fore.GREEN + self.Style.BRIGHT)
                return True
    ...
    def run_trading_cycle(self):
        try:
            ...
            if market_data:
                self.print_color(f"\n🔍 AGGRESSIVE AI SCANNING {len(market_data)} PAIRS FOR 15MIN ENTRIES...", self.Fore.BLUE + self.Style.BRIGHT)
                ...
                for pair in market_data.keys():
                    if self.can_open_new_trade(pair):
                        ...
                        if decision["action"] == "TRADE":
                            ...
                            self.print_color(f"🎯 QUALIFIED 15MIN: {pair} {direction_icon}", self.Fore.GREEN + self.Style.BRIGHT)
                ...
                if qualified_signals == 0:
                    ...
                    self.print_color(f"🎯 {qualified_signals} qualified 15min signals found", self.Fore.GREEN + self.Style.BRIGHT)
            else:
    ...
    def start_trading(self):
        ...
        while True:
            try:
                self.cycle_count += 1
                self.print_color(f"\n🎯 AGGRESSIVE 15MIN CYCLE {self.cycle_count}", self.Fore.RED + self.Style.BRIGHT)
                self.print_color("=" * 60, self.Fore.RED)
                self.run_trading_cycle()
                self.print_color(f"⏳ Waiting 2 minutes for next 15min cycle...", self.Fore.BLUE)
                time.sleep(120)  # 2 minutes between cycles for 15min timeframe
                
            except KeyboardInterrupt:
                self.print_color(f"\n🛑 AGGRESSIVE 15MIN TRADING STOPPED", self.Fore.RED + self.Style.BRIGHT)
...
class Aggressive15MinPaperTradingBot:
    ...
    def paper_execute_trade(self, decision):
        try:
            ...
            self.real_bot.print_color(f"\n🎯 15MIN PAPER TRADE EXECUTION", self.Fore.CYAN + self.Style.BRIGHT)
        ...
        try:
            ...
            for pair, trade in list(self.paper_positions.items()):
                ...
                if should_close:
                    ...
                    direction_icon = "🟢 LONG" if trade['direction'] == 'LONG' else "🔴 SHORT"
                    self.real_bot.print_color(f"\n🔚 15MIN PAPER TRADE CLOSED: {pair} {direction_icon}", pnl_color)
    ...
    def get_paper_portfolio_status(self):
        ...
        self.real_bot.print_color(f"\n📊 15MIN PAPER TRADING PORTFOLIO", self.Fore.CYAN + self.Style.BRIGHT)
        self.real_bot.print_color("=" * 70, self.Fore.CYAN)
    ...
    def run_paper_trading_cycle(self):
        try:
            ...
            if market_data:
                self.real_bot.print_color(f"\n🔍 AGGRESSIVE AI SCANNING FOR 15MIN PAPER TRADES...", self.Fore.BLUE + self.Style.BRIGHT)
                ...
                for pair in market_data.keys():
                    if pair not in self.paper_positions and len(self.paper_positions) < self.real_bot.max_concurrent_trades:
                        ...
                        if decision["action"] == "TRADE":
                            qualified_signals += 1
                            direction_icon = "🟢 LONG" if decision['direction'] == "LONG" else "🔴 SHORT"
                            self.real_bot.print_color(f"🎯 15MIN AI SIGNAL: {pair} {direction_icon}", self.Fore.GREEN + self.Style.BRIGHT)
                            self.paper_execute_trade(decision)
                            time.sleep(0.5)  # Small delay between paper executions
                
                if qualified_signals > 0:
                    self.real_bot.print_color(f"🎯 {qualified_signals} qualified 15min paper signals executed", self.Fore.GREEN + self.Style.BRIGHT)
    ...
    def start_paper_trading(self):
        ...
        while True:
            try:
                ...
                self.real_bot.print_color(f"\n🎯 15MIN PAPER CYCLE {self.paper_cycle_count}", self.Fore.CYAN)
                self.real_bot.print_color("=" * 60, self.Fore.CYAN)
                self.run_paper_trading_cycle()
                self.real_bot.print_color(f"⏳ Waiting 2 minutes...", self.Fore.BLUE)
                time.sleep(120)  # 2 minutes between cycles
                
            except KeyboardInterrupt:
                self.real_bot.print_color(f"\n🛑 15MIN PAPER TRADING STOPPED", self.Fore.RED + self.Style.BRIGHT)
                ...
                if total_trades > 0:
                    ...
                    self.real_bot.print_color(f"\n📊 FINAL 15MIN PAPER TRADING RESULTS", self.Fore.CYAN + self.Style.BRIGHT)
